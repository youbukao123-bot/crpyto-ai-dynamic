...
def get_klines(data_type: DataType, symbol, interval, start_time_str, end_time_str, output_path='day'):
    ...
    # 10050个， 需要调用 11次
    fetch_instance_num = (end_time - start_time) / interval_dict[interval]
    ...
    output_path = '../data/spot_{}_binance/{}_{}_{}_{}.csv'.format(output_path, symbol, interval, start_time_str, end_time_str)
    total_data.to_csv(output_path)
...
def get_kline_batch():
    with open('../data/fetch_data_exchange.txt') as f:
        for line in f.readlines():
            symbol = line.strip()
            if symbol:
                get_klines(DataType.SPOT, symbol, "15m", "2025-07-01_10_00", "2025-07-13_10_00", 'min')
def main():
    get_kline_batch()
