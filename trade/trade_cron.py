...
def trade():
    ...
    new_sell_order = http_client.place_order(symbol='solusdt'.upper(), order_side=OrderSide.SELL,
                                             order_type=OrderType.MARKET, quantity=6.61)
    print(new_sell_order)

    exit()

    check_order = http_client.get_all_order(symbol="solusdt".upper())
    print(check_order)
